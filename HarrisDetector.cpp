...
//All process options are done automatically by reading the lines of a .txt file you pass to the console while running it.
// I think we should avoid it and thus creating a sort of interaction with the user by letting him insert some (not all) parameters,
//such as K and the numberRingNeighboorhood. In my opinion this would make this part more easy for us.
//The fuction which defines the operation and parameters to execute
void HarrisDetector :: processOptions(){
    /*We evaluate the properties*/
    //typeNeighborhood
    string val = prop->getProperty("type-neighborhood");
    if(val.empty()){
        cout << "Option type-neighborhood was not set. RINGS is assumed." << endl;
        ...
        if(!val.compare("spatial"))
            ...
            cout << "Option type-neighborhood is not recognized. RINGS is assumed." << endl;
            typeNeighborhood = RINGS;
        }
    }
    ...
    if(typeNeighborhood == SPATIAL || typeNeighborhood == ADAPTIVE){
        ...
        }//When you don't write any number, the parameters are set to 2
        else
    ...
    if(val.empty()){
        ...
        k = 0.04; //That's the K of the Harris Operator
    }
    else{
        k = atof(val.c_str());
    }
...
void HarrisDetector :: detectInterestPoints(vector<Vertex>& interestPoints){
    assert(object); //did I read the Mesh? if no the program stops
    ...
    for(register int i = 0; i< object->getNumberVertices(); i++)
    {
        ...
        if(typeNeighborhood == ADAPTIVE){
            ...
            vertices[i].getNeighborhood(numberRingNeighbor, set, vertices); //calculates the n-th neighborhood
        }
        //r.tick();
        ...
        //Apply PCA to find a position of the point cloud so that the greatest scatter of the points is in the XY plane
        // The average of the 3 coordinates is already (0, 0, 0), so in reality it is not necessary to calculate it, we directly calculate the
        // covariance matrix
        double A[9];
        ...
        // With the covariance matrix, we calculate PCA
        ...
        // Order eigenvectors decreasingly by eigenvalue
        gsl_eigen_symmv_sort(eval, evec, GSL_EIGEN_SORT_VAL_DESC);

        // We take out the components of the new coordinate system
        ...
        // We perform the rotation, with the new coordinate system
        ...
        // We move all the points so that the point of analysis is at the origin of the XY plane. We only move in the X and Y coordinates
        double x = set[0]->getX();
        double y = set[0]->getY();
    ...
    if(typeSelection == FRACTION){
        ...
        // Select the points of greatest response

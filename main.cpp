...
int main(int argc, char *argv[])
{
    ...
    if(inp.fail()){
        ...
        // line 77 you are not doing any particular operation.
        //this is just the final combination (in one single object) of your points and parameters
		HarrisDetector hd(&mesh, &prop);
		
        //vector<type> name;
		vector<Vertex> interestPoints;
        interestPoints.clear(); //be sure that is empty
        Vertex* vertices = mesh.getVertices(); //i am pointing to the firts element of the vetixes copied in mesh
        //readed from the .txt file
		
        Clock r1; //i want to see how much time do i need to compute the operations.
		r1.tick();
		cout<<"->Interest points"<<endl;
        hd.detectInterestPoints(interestPoints); //applying the article we read ( we are going to implement only RING!!)
		r1.tick();
